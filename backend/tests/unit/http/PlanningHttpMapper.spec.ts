...
describe('PlanningHttpMapper - nombres públicos de unidades', () => {
  it('convierte calendario, alcance, eventos y comodines públicos a IDs internos', () => {
    ...
    expect(
      solicitud.calendarioOrigen.unidadesOperativas.map(({ nombre }) => nombre),
    ).toEqual(NOMBRES_INTERNOS);
    expect(solicitud.alcanceOperativo.unidadesOperativas).toEqual(
      NOMBRES_INTERNOS,
    );
    expect(solicitud.eventos.listar()[0]?.unidadOperativa).toBe(
      'CACAO PISTA',
    );
    expect(solicitud.comodines.listar()[0]?.unidadOperativa).toBe(
      'TRUCK STOP CAJA',
    );
  });
  ...
  it('acepta nombres públicos al exportar y convierte reemplazos al ID interno', () => {
    ...
    expect(
      solicitud.calendario.unidadesOperativas.map(({ nombre }) => nombre),
    ).toEqual(NOMBRES_INTERNOS);
  ...
  it('devuelve nombres públicos en calendarios, reemplazos y mensajes', () => {
    ...
    expect(dto.calendario.unidadesOperativas.map(({ nombre }) => nombre)).toEqual(
      NOMBRES_PUBLICOS,
    );
    expect(dto.cambios).toEqual([
      'Flexible Edwin transferido de CACAO C1 a CAJA CACAO.',
    ]);
    expect(dto.advertencias).toEqual([
      'Cobertura insuficiente en TRUCK STOP.',
    ]);
    expect(dto.reemplazos[0]?.unidadOperativa).toBe('CAJA CACAO');
    expect(conflicto.conflictos).toEqual([
      'CAJA TRUCK STOP debe planificarse con TRUCK STOP.',
    ]);
    expect(JSON.stringify({ dto, conflicto })).not.toContain('PISTA');
  });
});

function crearCalendarioDto(
...
function crearCalendarioDominio(
  nombres: ReadonlyArray<string>,
): Calendario {
